...
class AesAcc(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  override lazy val module = new AesAccModuleImp(this)
}

class AesAccModuleImp(outer: AesAcc)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  val s_idle :: s_l_key :: s_b_w_ready ::s_w_ready ::s_b_store:: s_store :: Nil = Enum(6)
  ...
  val key_exp_b = RegInit(rs2_rs1_exp)


  Aes.io.next := issue_next
  Aes.io.encdec := doEncipher_b
  Aes.io.key := key_exp_b
...
class aes_core extends BlackBox with HasBlackBoxResource{
  ...
  addResource("/vsrc/aes_rtl_128/aes_core.v")
  addResource("/vsrc/aes_rtl_128/aes_decipher_block.v")
  addResource("/vsrc/aes_rtl_128/aes_encipher_block.v")
  addResource("/vsrc/aes_rtl_128/aes_inv_sbox.v")
  addResource("/vsrc/aes_rtl_128/key_ass.v")
  addResource("/vsrc/aes_rtl_128/aes_sbox.v")
}
...
class Aes extends Module{
  ...
  val aes_core_inst = Module(new aes_core)
